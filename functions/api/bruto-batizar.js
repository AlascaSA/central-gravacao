...
export async function onRequest({ request, env }) {
  try {
    ...
    const b = (await (await fetch(`${S}/rest/v1/brutos?select=drive_id,card_id,nome,nome_original&drive_id=eq.${idq}`, { headers: hdr })).json())[0]
    ...
    if (!b.card_id) {
      ...
      // com roteiro → título do card; sem roteiro → "SR - {título do card}" (o "SR" fica só no arquivo)
      const base = (card.sem_roteiro ? 'SR - ' : '') + sanitizar(card.titulo || 'video')
