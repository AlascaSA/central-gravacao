...
export async function onRequest({ request, env }) {
  try {
    ...
    const b = (await (await fetch(`${S}/rest/v1/brutos?select=drive_id,card_id,nome,nome_original,tipo,ia_tipo&drive_id=eq.${idq}`, { headers: hdr })).json())[0]
    ...
    if (!b.card_id || ehDescarte) {
      ...
      const card = (await (await fetch(`${S}/rest/v1/cards?select=titulo,sem_roteiro,produto,team&id=eq.${encodeURIComponent(b.card_id)}`, { headers: hdr })).json())[0]
      ...
      const doTime = await (await fetch(`${S}/rest/v1/cards?select=id,titulo,produto,criado_em&team=eq.${encodeURIComponent(card.team)}`, { headers: hdr })).json()
      ...
      const irmaos = await (await fetch(`${S}/rest/v1/brutos?select=drive_id,criado,tipo,ia_tipo&card_id=eq.${encodeURIComponent(b.card_id)}&order=criado.asc,drive_id.asc`, { headers: hdr })).json()
